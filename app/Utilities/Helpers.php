...
class Helpers
{
    ...
    public static function generateKodeSubSkpd($instansi_id, $prefix = 'SUB', $delimiter = '-', $length = 3, $pad = '0'): string
    {
        ...
        $max = SubInstansi::query()->max('sub_instansi_id') !== 0 ? SubInstansi::query()->max('sub_instansi_id') + 1 : 1;
        $pad = Str::padLeft($max, (int) $length, $pad);

        return $prefix.$delimiter.$instansi_id.$pad;
    }

    public static function generateKode(string $prefix, $delimiter = '-', $length = 3, $pad = '0'): string
    {
        $prefix = $prefix ?? 'INS';
    ...
    public static function generateKodeKegiatan(string $prefix, $delimiter = '-', $length = 3, $pad = '0'): string
    {
        $prefix = $prefix ?? 'KEG';
        $max = Kegiatan::query()->max('kegiatan_id') !== 0 ? Kegiatan::query()->max('kegiatan_id') + 1 : 1;
        $pad = Str::padLeft($max, (int) $length, $pad);
        return $prefix.$delimiter.$pad;
    }

    public static function generateKodeSubKegiatan($kegiatan_id, $prefix = 'SUB', $delimiter = '-', $length = 3, $pad = '0'): string
    {
        $kegiatan_id = $kegiatan_id ?? 0;
        $kegiatan_id = Kegiatan::query()->find($kegiatan_id);
        $kegiatan_id = $kegiatan_id->kegiatan_id ?? 0;
        $max = SubKegiatan::query()->max('sub_kegiatan_id') !== 0 ? SubKegiatan::query()->max('sub_kegiatan_id') + 1 : 1;
        $pad = Str::padLeft($max, (int) $length, $pad);

        return $prefix.$delimiter.$kegiatan_id.$pad;
    }
